...
//package com.dbay.core.utils.serialize;
 ...
 * efficient than those found in org.json or net.sf.json. In profiling, the
 * performance of both of these libraries has been found to be woefully
 * inadequate for large scale deployments.
 *
 * The append*() methods can be used to serialize directly into an Appendable,
 * such as an output stream. This avoids unnecessary copies to intermediate
 * objects.
 *
 * To reduce output size, null values in json arrays and objects will always be
 * removed.
 */
...
//	}
    ...
    public static String serialize(Collection<?> collection) {
        StringBuilder buf = new StringBuilder(collection.size()
                * BASE_MULTIPLIER);
        try {
...
//	}
     ...
     *
     * @throws IOException
     *             If {@link Appendable#append(char)} throws an exception.
     */
    ...
    public static void append(Appendable buf, Object value) throws IOException {
        ...
        } else if (value instanceof CharSequence /* || value instanceof DateTime || value.getClass().isEnum() */) {
...
//			appendMultimap(buf, (Multimap<String, Object>) value);
     ...
     *             If {@link Appendable#append(char)} throws an exception.
     */
    public static void appendPojo(Appendable buf, Object pojo)
            throws IOException {
        ...
        for (Map.Entry<String, Method> entry : methods.entrySet()) {
            try {
                ...
                if (value != null) {
                    ...
                    // true
                    if (!attribute.equalsIgnoreCase("serialversionuid") && !((attribute.equals("isOwner") || attribute
                            .equals("isViewer")) && value.equals(Boolean.FALSE))) {
                        // Drop null values.
                        if (firstDone) {
                            buf.append(',');
                        } else {
...
//				e.printStackTrace();
     ...
     *             If {@link Appendable#append(char)} throws an exception.
     */
    public static void appendArray(Appendable buf, Object[] array)
            throws IOException {
        buf.append('[');
        boolean firstDone = false;
        for (Object o : array) {
     ...
     *             If {@link Appendable#append(char)} throws an exception.
     */
    public static void appendJsonArray(Appendable buf, JSONArray array)
            throws IOException {
        buf.append('[');
        boolean firstDone = false;
...
//		}
    ...
    public static void appendMultimap(Appendable buf,
                                      Multimap<String, Object> map) throws IOException {
...
//		}
     ...
     *             If {@link Appendable#append(char)} throws an exception.
     */
    public static void appendString(Appendable buf, CharSequence string)
            throws IOException {
